...
export default function Dashboard() {
  ...
  // Alten appointments per settore (vertical bar chart)
  const [altenAppuntamenti, setAltenAppuntamenti] = useState([]);
  ...
  useEffect(() => {
    ...
    // Fetch Alten appointments data
    fetch(`${import.meta.env.VITE_API_URL}/stats/alten-appuntamenti`, {
      headers: { Authorization: "Bearer " + localStorage.getItem("token") },
    })
      .then((res) => res.json())
      .then((data) =>
        setAltenAppuntamenti(
  ...
  const bankingClientiCount = useMemo(() => {
    const bankingClienti = clienti
      .filter((c) => c.settore === "Banking")
  ...
  const insuranceClientiCount = useMemo(() => {
    const insuranceClienti = clienti
      .filter((c) => c.settore === "Insurance")
      .map((c) => c.denominazione_cliente);
    const relevant = new Set(
      appuntamenti
        .filter(
  ...
  const chart2Data = useMemo(() => {
    const bankingIds = new Set(
      clienti.filter((c) => c.settore === "Banking").map((c) => c.id)
    );
    const insuranceIds = new Set(
      clienti.filter((c) => c.settore === "Insurance").map((c) => c.id)
    );
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-wrap justify-center gap-4">
        ...
        {/* Chart 3: Vertical Bar Chart for Alten appointments */}
        <div className="bg-gray-100 rounded-lg shadow p-8 flex flex-col items-center min-h-[400px] w-[30vw] justify-center">
          <div className="text-lg font-semibold text-blue-900 mb-4 text-center">
            Appuntamenti Alten per settore
          </div>
          <div style={{ width: "100%", height: 320, position: "relative" }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={altenAppuntamenti}
                margin={{ top: 10, right: 0, left: 0, bottom: 20 }}
              >
                <XAxis dataKey="settore" tick={{ fontSize: 12 }} />
                <YAxis
                  type="number"
                  allowDecimals={false}
                  tick={{ fontSize: 12 }}
                />
                <Tooltip />
                <Bar dataKey="count" fill="#1DC8DF" barSize={50} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>

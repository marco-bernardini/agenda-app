...
async function sendSdgReminders() {
  const appointments = await getUpcomingAppointments();

  for (const app of appointments) {
    const sdgers = await getSdgersForAppointment(app.id);
    for (const sdg of sdgers) {
      if (!sdg.email) continue;
      const html = formatAppointmentHtml(app, [sdg]);
      await transporter.sendMail({
        from: '"Agenda Financial Services" <marco.bernardini@sdggroup.com>',
        to: sdg.email,
        subject: `Promemoria appuntamento nei prossimi 7 giorni - ${app.denominazione_cliente}`,
        html,
        attachments: [
          {
            filename: 'logo.png',
      ...
      console.log(`Reminder sent to ${sdg.nominativo} (${sdg.email}) for appointment ${app.id}`);
    }
  }
}

// Optionally, schedule with cron or run once for testing
sendSdgReminders();